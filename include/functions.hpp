...
// Helper functions
inline void Instance::lock(std::string item) {
    locked.push_back(item);
}
inline void Instance::unlock(std::string item) {
    auto it = std::find(locked.begin(), locked.end(), item);
    if (it != locked.end()) locked.erase(it);
}
inline bool Instance::isLocked(std::string item) {
...
// Card Generators
inline std::string Instance::nextTarot(std::string source, int ante, bool soulable) {
...
inline std::string Instance::nextPlanet(std::string source, int ante, bool soulable) {
    std::string anteStr = std::to_string(ante);
...
inline std::string Instance::nextSpectral(std::string source, int ante, bool soulable) {
    std::string anteStr = std::to_string(ante);
    if (soulable) {
...
inline std::string Instance::nextPack(int ante) {
...
inline Pack packInfo(std::string pack) {
    if (pack[0] == 'M') {
...
inline std::vector<std::string> Instance::nextArcanaPack(int size, int ante) {
...
inline std::vector<std::string> Instance::nextCelestialPack(int size, int ante) {
    std::vector<std::string> pack;
    for (int i = 0; i < size; i++) {
...
inline std::vector<std::string> Instance::nextSpectralPack(int size, int ante) {
    std::vector<std::string> pack;
    for (int i = 0; i < size; i++) {
...
// Misc
inline bool Instance::isVoucherActive(std::string voucher) {
    return std::count(params.vouchers.begin(), params.vouchers.end(), voucher) > 0;
}
inline void Instance::activateVoucher(std::string voucher) {
...
inline std::string Instance::nextVoucher(int ante) {
    return randchoice("Voucher"+std::to_string(ante), VOUCHERS);
}

inline void Instance::setDeck(std::string deck) {
    params.deck = deck;
    if (deck == "Magic Deck") {
...
inline void Instance::setStake(std::string stake) {
    params.stake = stake;
}

inline std::string Instance::nextTag(int ante) {
    return randchoice("Tag"+std::to_string(ante), TAGS);
}

inline std::string Instance::nextBoss(int ante) {
